...
import com.panaceasoft.pskotlinmaterial.adapter.feature.filter.FeatureFilterDirectoryFilter1Adapter
import com.panaceasoft.pskotlinmaterial.repository.directory.PlaceRepository
import kotlinx.android.synthetic.main.feature_filter_directory_filter_1_activity.*
import java.util.*

class FeatureFilterDirectoryFilter1Activity : AppCompatActivity() {

    private lateinit var placeArrayList: ArrayList<Place>
    private lateinit var mAdapter: FeatureFilterDirectoryFilter1Adapter
    internal var numberOfColumns = 2


    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.feature_filter_directory_filter_1_activity)
    ...
    private fun initUI() {
        ...
        mAdapter = FeatureFilterDirectoryFilter1Adapter(placeArrayList)
    ...
    private fun initActions() {
        ...
        mAdapter.setOnItemClickListener(object : FeatureFilterDirectoryFilter1Adapter.OnItemClickListener{
            override fun onItemClick(view: View, obj: Place, position: Int) {
                Toast.makeText(this@FeatureFilterDirectoryFilter1Activity, "Selected : " + obj.name, Toast.LENGTH_LONG).show()
            }
        })
    }
