...
import kotlinx.android.synthetic.main.feature_grid_education_grid_4_item.view.*
 ...
 */
class FeatureGridEducationGrid4Adapter(private val itemList: List<EducationVideo>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    private lateinit var itemClickListener: FeatureGridEducationGrid4Adapter.OnItemClickListener
    ...
    fun setOnItemClickListener(mItemClickListener: FeatureGridEducationGrid4Adapter.OnItemClickListener) {
        this.itemClickListener = mItemClickListener
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.feature_grid_education_grid_4_item, parent, false)
    ...
    override fun onBindViewHolder(viewHolder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (viewHolder is FeatureGridEducationGrid4Adapter.PlaceViewHolder) {

            val item = itemList[position]

            viewHolder.titleTextView.text = item.title

            val videoTime = "Video : " + item.length
